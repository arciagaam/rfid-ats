    ...
    if (data?.schedule) {
        for (const day in data.schedule) {
            scheduleColumns.push(
                <ScheduleColumn key={day} day={day} scheduleList={data.schedule} />
            )
        }
    }
                    ...
                    <div className='flex flex-col gap-3 py-5'>
                        <div className='flex justify-between w-full'>
                            <h2 className='text-lg font-medium'>
                                Schedule of {data?.firstName} {data?.middleName ?? ''}{' '}
                                {data?.lastName}
                            </h2>
                        ...
                        {data?.schedule ? (
                            <div className='flex gap-2'>{scheduleColumns.map((col) => col)}</div>
                        ) : (
...
const ScheduleColumn = ({ ...props }) => {
    ...
    return (
        <div className='flex flex-col gap-3 flex-1'>
            ...
            {scheduleList[day] &&
                scheduleList[day].map((_: object, index: number) => (
                    <Cell key={index}>
    ...
    return (
        <div className='flex flex-col text-sm gap-3'>
            <div className='flex flex-col gap-1'>
                ...
                <input
                    className='bg-white border-b border-primary-blue-950/10'
                    readOnly
                    disabled
                    name={`${day}-${rowIndex}-subject`}
                    id={`${day}-${rowIndex}-subject`}
                    type='text'
                    value={scheduleList[day][rowIndex].subject}
                />
            </div>
            ...
            <div className='flex flex-col gap-1'>
                ...
                <input
                    ...
                    readOnly
                    disabled
                    name={`${day}-${rowIndex}-yearSection`}
                    id={`${day}-${rowIndex}-yearSection`}
                    type='text'
                    value={scheduleList[day][rowIndex].yearSection}
                />
            </div>
            ...
            <div className='flex flex-col gap-1'>
                ...
                <input
                    ...
                    readOnly
                    disabled
                    name={`${day}-${rowIndex}-timeStart`}
                    id={`${day}-${rowIndex}-timeStart`}
                    type='time'
                    value={scheduleList[day][rowIndex].timeStart}
                />
            </div>
            ...
            <div className='flex flex-col gap-1'>
                ...
                <input
                    ...
                    readOnly
                    disabled
                    name={`${day}-${rowIndex}-timeEnd`}
                    id={`${day}-${rowIndex}-timeEnd`}
                    type='time'
                    value={scheduleList[day][rowIndex].timeEnd}
                />
            </div>
        </div>
    )
...
const Cell = ({ children, ...props }) => {
    ...
    return (
        <div
            className={`relative flex items-center justify-center rounded-lg p-3 bg-white ring-1 ring-primary-blue-950/10 ${className}`}>
            {children}
        </div>
    )
}
